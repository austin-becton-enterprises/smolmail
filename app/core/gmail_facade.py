...
class GmailService:
    ...
    def read_most_recent_emails(self, label_ids: List[str] = [LABEL_INBOX, LABEL_UNREAD], max_results: int = 5) -> List[Dict]:
        """
        ...
        try:
            ...
            email_data = []
            for msg in messages:
                msg_detail = self.service.users().messages().get(userId=USER_ID_ME, id=msg[KEY_ID]).execute()
    ...
    def create_message(self, to: str, subject: str, body: str) -> Dict[str, str]:
        """
    ...
    def send_email(self, to: str, subject: str, body: str) -> Optional[Dict]:
        ...
        try:
            ...
            sent = self.service.users().messages().send(userId=USER_ID_ME, body=message).execute()
